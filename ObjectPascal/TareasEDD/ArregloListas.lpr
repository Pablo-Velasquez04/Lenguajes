...
begin
  ...
  //Solicitamos cantidad de tareas
  Write('Ingrese la cantidad de tareas (n): ');
  ReadLn(n);

  //Redimensionamos el arreglo dinámico
  SetLength(Tareas, n);

  //Inicializamos las listas de días
  for i := 0 to 4 do
    Dias[i] := TStringList.Create;

  try
    //Capturamos información de cada tarea
    WriteLn;
    ...
    //Distribuimos tareas por días usando módulo 5
    WriteLn;
    ...
    //Liberaramos memoria de las listas
    for i := 0 to 4 do
      Dias[i].Free;
  end;
